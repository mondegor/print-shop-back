...
func (uc *FormElement) checkForm(ctx context.Context, item *entity.FormElement) error {
	if item.FormID == uuid.Nil {
		return module.ErrSubmitFormRequired
	}
	...
	if err != nil {
		...
		return uc.errorWrapper.Wrap(err, "formId", item.FormID)
	}
...
func (uc *FormElement) getFormID(ctx context.Context, itemID uint64) (uuid.UUID, error) {
	...
	if item.FormID == uuid.Nil {
		return uuid.Nil, errors.NewInternalError(
			"item.FormID is uuid.Nil",
			"model", entity.ModelNameFormElement,
		)
	}
