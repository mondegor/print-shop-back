...
func NewPaperFacture(
	...
	return &PaperFacture{
		storage:       storage,
		eventEmitter:  mrevent.NewSourceEmitter(eventEmitter, entity.ModelNamePaperFacture),
...
func (uc *PaperFacture) GetItem(ctx context.Context, itemID uint64) (entity.PaperFacture, error) {
	if itemID == 0 {
		return entity.PaperFacture{}, mr.ErrUseCaseEntityNotFound.New()
	}

	item, err := uc.storage.FetchOne(ctx, itemID)
	if err != nil {
		return entity.PaperFacture{}, uc.errorWrapper.WrapErrorNotFoundOrFailed(err, "itemId", itemID)
	}
